...
{
    /// <summary>
    /// DataPage の ViewModel。直近のディレクトリ一括タグ付け実行結果の表示と、
    /// 任意ディレクトリのタグ集計レポート（tags_report.txt）の生成・表示を担当する。
    /// </summary>
    public partial class DataViewModel : ObservableObject, INavigationAware
    {
        ...
        /// <summary>
        /// DI コンテナから設定・結果共有ストア・スナックバーサービスを受け取って初期化する。
        ...
        private async Task GenerateReportAsync()
        {
            ...
            try
            {
                ...
                ReportStatusText = string.Format(
                    LocalizationManager.Instance["Data_ReportGeneratedFormat"], ReportEntries.Count, reportPath);
