...
### フェーズ28: ReportPage へのタグ使用画像一覧表示機能の追加（`feature/report-tag-usage-images` ブランチ、実装完了）

「ListView でタグが選択された時に、そのタグがどの画像で使われているかを表示したい」というユーザー要望を受けて追加した。表示箇所は既存の ListView 直下に用意されていた空の Border（`ReportPage.xaml` の `Grid.Row="7"`）を使い、`ItemsControl`（`WrapPanel` で水平方向に折り返し）で画像ファイル名を表示する方式とした。

- `ViewModels/Pages/ReportViewModel.cs`:
  - `SelectedTag`（`[ObservableProperty]`、`TagCountEntry?`）を新設し、`ReportPage.xaml` の `ui:ListView.SelectedItem` と双方向バインドする。`partial void OnSelectedTagChanged` で `LoadTagUsageImagesAsync(value)` を呼び出す（fire-and-forget）
  - `TagUsageLoadTask`（`public Task`、既定 `Task.CompletedTask`）を新設し、`OnSelectedTagChanged` が開始したタスクをこのプロパティへ保持する。`SelectedTag` の変更がフィールドへの非同期処理を fire-and-forget で起動する初めてのケースのため、テストから完了を確実に待てるようこの形にした（`vm.SelectedTag = tag; await vm.TagUsageLoadTask;` で検証できる）
  - `TagUsageImages`（`ObservableCollection<string>`）: `LoadTagUsageImagesAsync` が `SelectedTag` を使用している画像のファイル名（ファイル名昇順）を反映する。`SelectedTag` が null、または `ReportDirectory` が未設定/未存在の場合は空にする
  - 画像収集ロジック（`CollectImagesUsingTag`、対応拡張子 `.jpg`/`.jpeg`/`.png`/`.webp`、`ReportRecursive` に応じた探索、同名 `.txt` の読み込み・`SplitTags` によるカンマ区切り分割・タグの大文字小文字無視の一致判定）は `GalleryViewModel.CollectEntries`/`SplitTags` と同じ内容を `ReportViewModel` 内に複製した（`ComfyUILibs.Services.CaptioningService` 側が internal のため参照できず GUI 側に閉じる、というフェーズ13以来の既存方針を踏襲）。`Task.Run` でバックグラウンド実行する
  - `GenerateReportAsync` の冒頭（既存の `_allReportEntries`/`ReportEntries`/`TagList`/`FilterText`/`ReportStatusText` リセット箇所）に `TagUsageImages.Clear()`/`SelectedTag = null` を追加し、レポート再生成のたびに前回の選択・使用画像一覧をリセットするようにした
- `Views/Pages/ReportPage.xaml`:
  - `ui:ListView` に `SelectedItem="{Binding ViewModel.SelectedTag, Mode=TwoWay}"` を追加
  - 既存の空 `Border`（`Grid.Row="7"`）の中に、未選択時のプレースホルダー文言（新設 `Report_TagUsageImagesPrompt`、`DataTrigger Binding="{Binding ViewModel.SelectedTag}" Value="{x:Null}"` で表示切替、`GalleryPage.xaml` の `Gallery_SelectImagePrompt` と同じパターン）と、`ItemsControl`（`ItemsPanel` に `WrapPanel Orientation="Horizontal"`、`ScrollViewer MaxHeight="160"` で包む）による画像ファイル名のチップ一覧を配置した
- `Resources/Strings.resx`/`Strings.en.resx` に `Report_TagUsageImagesPrompt` を追加
- `ComfyUICaptioningToolTests`: `ViewModels/Pages/ReportViewModelTests.cs` に6件追加（`SelectedTag` 設定で使用画像がファイル名昇順で反映されること・`null` 設定で空になること・タグ一致が大文字小文字無視であること・該当画像なし時は空のままであること・`SelectedTag` を変更すると前回の一覧が置き換わること・`GenerateReportCommand` 再実行で `SelectedTag`/`TagUsageImages` がリセットされること）。全290件中288件パス確認済み（`ComfyUICaptioningToolTests.exe` 直接実行で確認。失敗した2件は `CopyTagsToClipboardCommand` 関連で、フェーズ27で記録済みのこのマシン特有のクリップボードアクセス不可という既知の環境依存事象であり、本フェーズの変更とは無関係）
- 実アプリでの目視確認は、過去のフェーズから繰り返し発生している環境依存の制約（座標指定でのクリック操作・スクリーンショットが無関係な別ウィンドウを誤操作/誤取得する）により今回も断念し、ユニットテストとコードレビュー（`GalleryPage.xaml` の `SelectedImage` null 判定と同じ `DataTrigger` パターンであること）で代替した

### フェーズ29: GalleryPage 右ペインのタグ追加入力欄で既存タグ入力時に選択状態へ切り替える機能の追加（`feature/gallery-tag-input-select-existing` ブランチ、実装完了）

「右ペインのタグ追加入力欄に既存タグ名を入力しても現状は何も反映されない（`AddTag` が重複として無視するのみ）ので、既存タグが入力された場合はそのタグのトグルボタンを選択状態にしてほしい。選択状態になった場合は削除・並び替えボタンも有効になるようにしてほしい」というユーザー要望を受けて追加した。

- `Models/GalleryImageEntry.cs` に `private bool SelectExistingTag(string input)` を新設。入力を trim し、`Tags` と大文字小文字無視で一致する既存タグがあれば、その既存タグの表記（入力欄のタグではなく `Tags` 側の実際の文字列）を `SelectedTags` へ追加（未選択の場合のみ、既に選択済みなら何もしない＝トグルではなく「選択オン」の冪等な操作）し `true` を返す。一致しなければ `false` を返す
- `AddNewTagAsync`（末尾に追加）/`AddNewTagToStartAsync`（先頭に追加、いずれもフェーズ14/18で追加済みのカード単位のタグ追加コマンド）の冒頭で、まず `SelectExistingTag` を呼び出し、一致した場合はそこで処理を終える（`AddTag` を呼ばない＝`.txt`/`captioning_config_result.json`/`gallery_edit_log.jsonl` への書き込みは発生させない、TagList 更新コールバックも呼ばない）。一致しなかった場合のみ従来通り `AddTag` を呼ぶ
- 「選択状態になった場合は削除・順序変更ボタンも有効になる」という要件は追加実装が不要だった: `SelectedTags` はフェーズ22/26で導入済みの `ObservableCollection<string>` で、その `CollectionChanged` ハンドラーが `RemoveSelectedTagsCommand`/`MoveSelectedTagsToStartCommand`/`MoveSelectedTagsToEndCommand`/`MoveSelectedTagsUpCommand`/`MoveSelectedTagsDownCommand`（いずれも `CanExecute = nameof(HasSelectedTags)`）の `NotifyCanExecuteChanged` を既に呼び出す実装になっているため、`SelectExistingTag` で `SelectedTags` に追加するだけでこれらのボタンは自動的に活性化する
- XAML（`Views/Pages/GalleryPage.xaml`）の変更は行っていない。タグ追加入力欄・ボタン（`AddNewTagCommand`/`AddNewTagToStartCommand` にバインド済み）はそのままで、コマンド本体の挙動変更のみで要件を満たせるため
- `ComfyUICaptioningToolTests`: `Models/GalleryImageEntryTests.cs` に6件追加（`AddNewTagCommand`/`AddNewTagToStartCommand` それぞれで既存タグ入力時に追加ではなく選択状態になり入力欄がクリアされること・既存タグ入力時は `.txt` を書き換えない（新規作成もしない）こと・既に選択済みのタグを再入力しても選択状態のまま変化しないこと・既存タグ入力時は TagList 更新コールバックが呼ばれないこと・既存タグ入力による選択で `RemoveSelectedTagsCommand.CanExecute` が `false`→`true` に変わること）。全296件中294件パス確認済み（`ComfyUICaptioningToolTests.exe` 直接実行で確認。失敗した2件は `CopyTagsToClipboardCommand` 関連で、フェーズ27/28で記録済みのこのマシン特有のクリップボードアクセス不可という既知の環境依存事象であり、本フェーズの変更とは無関係）
- 実アプリでの目視確認は、過去のフェーズから繰り返し発生している環境依存の制約（座標指定でのクリック操作・スクリーンショットが無関係な別ウィンドウを誤操作/誤取得する）により今回も断念し、ユニットテストとコードレビュー（既存の `AddTag`/`ToggleTagSelection` と同じ `SelectedTags` 操作パターンであること）で代替した

### フェーズ30: GalleryPage 右ペインのタグ追加入力欄を ui:AutoSuggestBox 化（実装完了）

「右ペインのタグ追加入力欄（`ui:TextBox`）を `ui:AutoSuggestBox` に変更し、選択中の画像に存在するタグを候補表示してほしい」というユーザー要望を受けて追加した。フェーズ29で「既存タグ名を入力すると追加ではなく選択状態に切り替わる」挙動を実装済みだったため、本フェーズは候補一覧を見えるようにする UI 面の変更のみとなる。

- `Views/Pages/GalleryPage.xaml`: 右ペインのタグ追加入力欄（`Grid.Row="3"` の `Grid.Column="1"`）を `ui:TextBox` から `ui:AutoSuggestBox` に変更した。`Text` は従来通り `NewTagInput` に `TwoWay`/`UpdateSourceTrigger=PropertyChanged` でバインドし、`OriginalItemsSource` を `Tags`（この右ペインの `DataContext` である選択中の `GalleryImageEntry` 自身の `Tags`）にバインドした。この右ペインは `Views/Pages/GalleryPage.xaml` の `Grid DataContext="{Binding ViewModel.SelectedImage}"`（フェーズ23で導入）配下にあるため、`Tags` は「選択中の画像 1 枚に既に付いているタグ」のみを指す（一括タグ操作カードの `ui:AutoSuggestBox`（フェーズ17）が `ViewModel.TagList`＝対象ディレクトリ全体の `tags_report.txt` 由来のタグ一覧を候補にするのとは異なる）
- `Enter` キーでの追加（`AddNewTagCommand`）用の `KeyBinding` は `ui:TextBox.InputBindings` から `ui:AutoSuggestBox.InputBindings` にそのまま移設した（`InputBindings` は `UIElement` 共通のコレクションのため、コントロール種別を変えても同じ書き方で動作する）
- `Models/GalleryImageEntry.cs`/`ViewModels/Pages/GalleryViewModel.cs` の変更は行っていない。候補入力から既存タグ名がそのまま `NewTagInput` に入るだけで、フェーズ29で実装済みの `SelectExistingTag`（大文字小文字無視の一致判定で選択状態に切り替える）がそのまま機能する
...
### フェーズ32: タグ付けバックエンド選択 GUI 配線（実装完了、コミット保留）

フェーズ31で追加した `WdV3TimmTaggerRunner`（`ComfyUILibs`）・`wdv3-timm` サブモジュールを実際に GUI から利用できるようにする配線。ユーザー指示により本フェーズの変更は実装完了後もコミットしていない（作業ツリーに留めている）。

- **`CaptioningService`/`ICaptioningService` 層の抽象化対応**:
  - `Services/CaptioningServiceAdapter.cs` のコンストラクター引数を `Wd14TaggerRunner` の具象型から `ITaggerRunner`（`ComfyUILibs.Services`）に変更（`ICaptioningService.cs` 自体は変更不要）
  - `Services/TaggerRunnerFactory.cs`（新設） — `captioning_config.json` のパスと `TaggerBackend` から `ITaggerRunner` の実装（`WdV3Timm` なら `WdV3TimmTaggerRunner`、それ以外の既定 `ComfyUI` なら `Wd14TaggerRunner`）を構築する静的ファクトリー
- **`Models/TaggerBackend.cs`（新設）** — `ComfyUI`/`WdV3Timm` の列挙型。`Models/AppConfig.cs` に `TaggerBackend`（既定 `TaggerBackend.ComfyUI`、既存の挙動を完全に維持）プロパティを追加した
- **`MainPageViewModel.cs`/`ReportViewModel.cs`/`GalleryViewModel.cs` の `TryLoadRunner`（同期）を `TryLoadRunnerAsync`（非同期）に変更**: `Config.Data.ConfigPath`・`Config.Data.TaggerBackend` から `TaggerRunnerFactory.Create` で `ITaggerRunner` を構築する。既存の `_taggerRunner` が `IAsyncDisposable`（`WdV3TimmTaggerRunner`）の場合、再構築前に `DisposeAsync()` を呼んで常駐サーバープロセスを終了させてから差し替える（`Wd14TaggerRunner` は `IAsyncDisposable` を実装しないため ComfyUI バックエンド利用時は実質的な挙動変化なし）
  - **`MainPageViewModel.RunAsync` の追加対応**: `_taggerRunner`（特に `WdV3TimmTaggerRunner`）は `ProcessDirectoryAsync` の実行中に常駐サーバープロセスを起動する。ページ遷移（`OnNavigatedToAsync`）のたびにしか `TryLoadRunnerAsync` が呼ばれない設計のままだと、同じページに留まったまま複数回「実行」した場合に前回のプロセスが破棄されずリークし、かつ 2 回目の実行が「破棄済みプロセスへのリクエスト」で全画像エラーになってしまう。これを避けるため、`RunAsync` の `finally`（`SaveResultLogAsync` の後）で `TryLoadRunnerAsync(showErrorSnackbar: false)` を呼び、実行完了のたびに Runner を破棄・再構築するようにした（`showErrorSnackbar` 引数はページ遷移時の通常読み込み失敗表示と、実行完了時の内部的な再構築失敗表示を区別するために追加。後者は今回の実行結果通知と重複させないため既定で無音にした）
  - `ReportViewModel`/`GalleryViewModel` は `GenerateReportAsync`（タグ本体を読まず `.txt` 集計のみ）しか呼ばないため実際には `TagAsync` が呼ばれず常駐プロセスは起動しないが、将来の変更に備えて同じ破棄ガードを入れている
- **`ConfigViewModel.cs` に wdv3-timm セクションの編集項目を追加**: `WdV3TimmExePath`、ファイル選択コマンド `BrowseWdV3TimmExePathCommand`。
  - **設計修正（ユーザー指摘を受けて実装完了後に変更）**: 当初は `WdV3TimmModel`/`WdV3TimmGeneralThreshold`/`WdV3TimmCharacterThreshold` を wdv3_timm セクション独自のフィールドとして追加していたが、「保存した wdv3-timm 設定が captioning_config.json に反映されない」との報告と、「ComfyUI 版と wdv3-timm 版でモデル名・しきい値を別々に管理すると設定がずれるため、wdv3-timm 側は wd14_tagger のものを使うようにしてほしい」という要望を受けて設計変更した（詳細は `ComfyUILibs/.claude/implementation_status.md` のフェーズ8を参照。`WdV3TimmConfig` から `Model`/`GeneralThreshold`/`CharacterThreshold` を削除し `ExePath` のみに、`WdV3TimmModelMap` で `wd14_tagger.model_name` を wdv3-timm の `--model` 値へ変換する設計にした）。この変更に伴い `ConfigViewModel` 側の `WdV3TimmModel`/`WdV3TimmGeneralThreshold`/`WdV3TimmCharacterThreshold` プロパティと `WdV3TimmModelList` は削除した
  - `comfyui_url`/`wd14_tagger` と `wdv3_timm` は排他ではなく独立したセクションとして扱う方針を維持しつつ、`wd14_tagger` セクションの出力条件を `ComfyUiUrl` 非空から `ModelName` 非空に変更した（wdv3-timm のみを使う場合、`ComfyUiUrl` は空でも `ModelName`・しきい値は必要になったため）。`WdV3Timm` セクションは `WdV3TimmExePath` 非空で出力する（変更なし）
  - `Save()` は `Config.Data.TaggerBackend` が指す方のセクションのみ必須として検証する（`ComfyUI` なら `ConfigLoader.ValidateWd14TaggerConfig`、`WdV3Timm` なら `ConfigLoader.ValidateWdV3TimmConfig`。後者は内部で `ValidateWd14TaggerConfig` も呼ぶため、`WdV3Timm` 選択時も `ModelName`・しきい値・`WdV3TimmModelMap` で変換可能なモデル名が必要になる）
- **`SettingsViewModel.cs`/`SettingsPage.xaml` にタグ付けバックエンド選択を追加**: `SelectedTaggerBackend`/`TaggerBackendList`（`SelectedTheme`/`ThemeList` と同じ ComboBox パターン、変更は `partial void OnSelectedTaggerBackendChanged` で即座に `Config.Data.TaggerBackend` へ反映）。`SettingsPage.xaml` に「タグ付けバックエンド」カード（アイコン `ArrowSwap24`）を「表示」の次・「ComfyUI 接続」の前に配置した
- **`ConfigPage.xaml` に「wdv3-timm 設定（ローカルプロセス版）」カードを追加**: 「WD14 モデル設定」カードと同じ見た目（200/* 2列 Grid、アイコン無し）。実行ファイルパス（`ui:TextBox` + ファイル選択ボタン）のみを持ち、カード下部に「モデル・しきい値は上記の『WD14 モデル設定』を使用します。」という案内文（`Config_WdV3TimmSharesWd14Notice`）を表示する
- `Resources/Strings.resx`/`Strings.en.resx` に `Settings_TaggerBackendSectionLabel`/`Settings_TaggerBackendLabel`/`Config_WdV3TimmSectionLabel`/`Config_WdV3TimmExePathLabel`/`Config_WdV3TimmExePathPlaceholder`/`Config_WdV3TimmExePathDialogTitle`/`Config_WdV3TimmExePathEmptyError`/`Config_WdV3TimmSharesWd14Notice` を追加（`Config_WdV3TimmModelLabel` は設計修正で不要になったため追加しなかった）
- **追加修正（ユーザー指摘を受けて）**: `ConfigPage` のモデル名（`Config_ModelNameLabel`）入力欄が自由入力の `ui:TextBox` のままだと、`WdV3TimmModelMap` で変換できない値を入力できてしまう（wdv3-timm バックエンド選択時に保存が失敗する）ため、`ComboBox` に変更した。選択肢は `ComfyUILibs.Services.WdV3TimmModelMap.SupportedWd14ModelNames`（`wd-vit-tagger-v3`/`wd-swinv2-tagger-v3`/`wd-convnext-tagger-v3`/`wd-eva02-large-tagger-v3`/`wd-vit-large-tagger-v3` の5件、`WdV3TimmModelMap` の対応表と自動的に一致する）を `ConfigViewModel.ModelNameList` として公開しバインドする。`ComfyUILibs` 側の `WdV3TimmModelMap` に `SupportedWd14ModelNames`（`Wd14ToWdV3Timm.Keys`）を新設し、GUI 側で同じ一覧を二重管理しないようにした。使われなくなった `Config_ModelNamePlaceholder` リソースキーは削除した
- `ComfyUICaptioningToolTests`:
  - `Services/TaggerRunnerFactoryTests.cs`（新設、5件） — `TaggerBackend` ごとの実装型・wdv3_timm/wd14_tagger 各セクション欠落時の例外を検証
  - `Models/AppConfigTests.cs` に `TaggerBackend` 既定値・PropertyChanged のテストを追加（2件）
  - `ViewModels/Pages/SettingsViewModelTests.cs` に `TaggerBackendList`/`SelectedTaggerBackend` のテストを追加（6件）
  - `ViewModels/Pages/ConfigViewModelTests.cs` に wdv3-timm フィールド（`ExePath` のみ）の読み込み・既定値・`TaggerBackend` ごとの `Save()` 挙動（wd14_tagger 共用・`ModelName` 欠落/未対応モデル名/しきい値範囲外時のエラー表示を含む）のテストを追加（9件）
  - `ViewModels/Pages/MainPageViewModelTests.cs` に `TaggerBackend.WdV3Timm` 選択時の `IsConfigLoaded`・`captioningServiceFactory` へ渡される Runner の型・実行完了後の破棄・再構築（連続実行可能なこと）を検証するテストを追加（4件、`WriteWdV3TimmConfigFile` ヘルパーは wd14_tagger セクションも書き出すよう修正）
  - `MainPageViewModelTests.cs`/`ReportViewModelTests.cs`/`GalleryViewModelTests.cs` の `CreateVm` ヘルパー（テスト用ファクトリー）の型を `Func<Wd14TaggerRunner, ...>` から `Func<ITaggerRunner, ...>` に更新
  - `ConfigViewModelTests.cs` に `ModelNameList`（5件のモデル名を含むこと）のテストを追加
  - 全323件中321件パス確認済み（`ComfyUICaptioningToolTests.exe` 直接実行で確認。失敗した2件は `CopyTagsToClipboardCommand` 関連で、フェーズ27以降記録済みのこのマシン特有のクリップボードアクセス不可という既知の環境依存事象であり、本フェーズの変更とは無関係）
- 実アプリでの目視確認は、過去のフェーズから繰り返し発生している環境依存の制約（座標指定でのクリック操作・スクリーンショットが無関係な別ウィンドウを誤操作/誤取得する）により今回も断念し、ユニットテストとコードレビュー（既存の `SettingsPage`/`ConfigPage` カードパターンと同一の XAML 構造であること）で代替した
- **本フェーズのスコープ外**: `MainPage`/`GalleryPage`/`ReportPage` にページ単位でどのバックエンドが使われているかを示す UI 表示は追加していない（バックエンドは `SettingsPage` の一括切り替えのみ）。wdv3-timm 実行ファイルの実機（実際の python プロセス）を使った実行確認は、`FakeCaptioningService` が `ITaggerRunner.TagAsync` を実際には呼ばない設計のため本フェーズのユニットテストでは検証できておらず、別途手動確認が必要

- **本節の `ConfigViewModel.WdV3TimmExePath`/`BrowseWdV3TimmExePathCommand`・`ConfigPage.xaml` の「wdv3-timm 設定（ローカルプロセス版）」カード・`WorkflowConfig.WdV3Timm`（`ComfyUILibs`）は、以下の追加修正（ユーザー指示「wdv3_timm.exe の実行ファイルパスは、ツールの同階層固定としてください。その代わり、venv と exe のビルドを自動的に行うボタンを追加してください。」を受けたもの）で廃止済み。**
  - **`ComfyUILibs`**: `WorkflowConfig.WdV3Timm`/`WdV3TimmConfig` クラスを削除し、`wdv3_timm` セクション自体を `captioning_config.json` から廃止した。`Services/WdV3TimmPaths.cs`（新設）に固定パス規約を集約: `RootDirectory = Path.Combine(AppContext.BaseDirectory, "wdv3-timm")`、`ExeFilePath = Path.Combine(RootDirectory, "wdv3_timm.exe")`（`wdv3-timm` サブモジュールのフォルダ構成が `wdv3_timm.exe` を `.venv`/`wdv3_timm.py` と同階層に置く前提のため、`wdv3-timm/` サブフォルダごと固定した）。`WdV3TimmTaggerRunner.EnsureStartedAsync` は `--model`/`-g`/`-c` を（従来通り）`Wd14Tagger` セクションから組み立て、起動時の実行ファイルパスは config 由来ではなく `WdV3TimmPaths.ExeFilePath` を直接使うよう変更した。実行ファイル未配置時のエラーハンドリングは新規の存在チェックを追加せず、既存の `WdV3TimmProcessClient.StartAsync` 内の `Win32Exception` → `ComfyUIException` ラップ（プロセス起動失敗時の既存経路）にそのまま委ねる設計とした（`AppContext.BaseDirectory` はプロセス全体で共有されるため、テスト時に実ファイルを置く固定パスを用意するとテストクラス間で干渉するリスクがあり、それを避けるための判断）。`ConfigLoader.ValidateWdV3TimmConfig` も `ExePath`/`wdv3_timm` セクションのチェックを削除し、`ValidateWd14TaggerConfig` + `WdV3TimmModelMap.TryGetWdV3TimmModel` によるモデル名マッピング検証のみに簡素化した
  - **`ComfyUICaptioningTool`**: `ConfigViewModel`/`ConfigPage.xaml` から `WdV3TimmExePath` 関連の入力欄・コマンドを完全に削除し、「WD14 モデル設定」カードの下部に注記文（`Config_WdV3TimmSharesWd14Notice`、wording 更新）のみを残した。代わりに `SettingsPage.xaml` に「wdv3-timm 実行環境（ローカルプロセス版）」カード（`Settings_WdV3TimmSectionLabel`）を新設し、(1) 準備状態表示（`SettingsViewModel.WdV3TimmStatusText`、`IsWdV3TimmExeReady` に応じて `Settings_WdV3TimmReady`/`Settings_WdV3TimmNotReady` を切替）、(2) 「ビルド」ボタン（`BuildWdV3TimmCommand`、実行中は `ui:ProgressRing` を表示）、(3) 説明文（`Settings_WdV3TimmBuildDescription`）、(4) 実行ログ表示（`ListBox`、`WdV3TimmBuildLogEntries`、`DataPage.xaml` の 1 ファイルごとの処理結果ログと同じ見た目）を配置した
  - **`Services/IWdV3TimmBuildService.cs`/`WdV3TimmBuildService.cs`（新設）**: `bool IsExeReady`（`File.Exists(WdV3TimmPaths.ExeFilePath)`）と `Task<bool> BuildAsync(IProgress<string> outputProgress, CancellationToken)` を持つ。`BuildAsync` は `wdv3-timm` フォルダ同梱の `setup.bat` → `build_exe.bat` を `cmd.exe /c` 経由で順に実行し（後者は前者が終了コード 0 の場合のみ実行）、`RedirectStandardOutput`/`Error` + `OutputDataReceived`/`ErrorDataReceived` で標準出力・エラー出力を 1 行ごとに `outputProgress.Report` へ流す（`MainPageViewModel` の `IProgress<CaptioningProgress>` 購読と同じ「進捗をリアルタイムでログへ追記する」パターン）。ルートフォルダ／スクリプトファイルが見つからない場合はプロセスを起動せずメッセージのみ報告して `false` を返す。コンストラクターは `WdV3TimmBuildService()`（既定、`WdV3TimmPaths.RootDirectory` を使用）と `WdV3TimmBuildService(string rootDirectory)`（テスト用に対象フォルダを差し替え可能）の 2 種を公開した（`ComfyUILibs` 側で採用している `internal` コンストラクター + `InternalsVisibleTo` の境界パターンは本プロジェクトでは使っていないため、それに合わせて `public` のままとした）
  - **`SettingsViewModel.cs`**: コンストラクターに `ISnackbarService`（必須、ビルド完了時の成功/失敗スナックバー表示用）と `IWdV3TimmBuildService?`（既定 `new WdV3TimmBuildService()`）を追加。`IsWdV3TimmExeReady`/`WdV3TimmStatusText`/`IsBuildingWdV3Timm`/`WdV3TimmBuildLogEntries` を新設し、`OnNavigatedToAsync` のたびに（初回限定だったテーマ/言語/バックエンド初期化とは異なり毎回）`IsWdV3TimmExeReady` を再チェックする。`BuildWdV3TimmCommand`（`CanExecute`: `!IsBuildingWdV3Timm`）は `MainPageViewModel.SynchronousProgress<T>` と同じ考え方の同期 `IProgress<string>` 実装（`SettingsViewModel` 内に private nested class として複製）でビルド出力をログへ即時反映し、完了後に `IsWdV3TimmExeReady` を再取得してスナックバー表示・`IsBuildingWdV3Timm` を `false` に戻す
  - `App.xaml.cs` に `services.AddSingleton<IWdV3TimmBuildService, WdV3TimmBuildService>();` を追加
  - `Resources/Strings.resx`/`Strings.en.resx`: `Config_WdV3TimmSectionLabel`/`Config_WdV3TimmExePathLabel`/`Config_WdV3TimmExePathPlaceholder`/`Config_WdV3TimmExePathDialogTitle`/`Config_WdV3TimmExePathEmptyError`/`Config_ModelNamePlaceholder` を削除。`Settings_WdV3TimmSectionLabel`/`Settings_WdV3TimmReady`/`Settings_WdV3TimmNotReady`/`Settings_WdV3TimmBuildButtonContent`/`Settings_WdV3TimmBuildDescription`/`Settings_WdV3TimmBuildSuccess`/`Settings_WdV3TimmBuildFailure`/`Settings_WdV3TimmBuildRootNotFoundFormat`/`Settings_WdV3TimmBuildScriptNotFoundFormat`/`Settings_WdV3TimmBuildScriptStartingFormat`/`Settings_WdV3TimmBuildScriptFailedFormat` を追加
  - `ComfyUILibsTests`: `ConfigLoaderTests.cs`/`WdV3TimmTaggerRunnerTests.cs`（`ExePath`/`wdv3_timm` セクション関連のテストを削除・`StartedExePath` が `WdV3TimmPaths.ExeFilePath` と一致することを検証するテストへ更新）/`WdV3TimmModelMapTests.cs`（`SupportedWd14ModelNames` のテストを追加）を更新。227件、全件パス確認済み
  - `ComfyUICaptioningToolTests`: `ConfigViewModelTests.cs`（ExePath 関連テストを削除、`TaggerBackend.WdV3Timm` 選択時の `Save()` を wd14_tagger 共用の観点で再検証）、`MainPageViewModelTests.cs`/`TaggerRunnerFactoryTests.cs`（`WriteWdV3TimmConfigFile` ヘルパーから `wdv3_timm` セクションを削除）、`SettingsViewModelTests.cs`（コンストラクター変更に伴う `CreateVm`/`RunOnSta` ヘルパー追加・ビルド機能の新規テスト約15件）、`Fakes/FakeWdV3TimmBuildService.cs`（新設）、`Services/WdV3TimmBuildServiceTests.cs`（新設、7件。隔離した一時フォルダに軽量なダミー `.bat` を配置し、実際の `cmd.exe` プロセス起動・標準出力ストリーミング・終了コード判定・`setup.bat`→`build_exe.bat` の逐次実行〔前者失敗時は後者を実行しない〕を検証。フェイクではなく実サービスクラスをそのまま対象にできるのは、上記の `public WdV3TimmBuildService(string rootDirectory)` コンストラクターがあるため）を追加。全337件中334件パス確認済み（`ComfyUICaptioningToolTests.exe` 直接実行で確認。失敗した3件のうち2件は既知のクリップボードアクセス不可、残り1件（`DataViewModelTests.OnNavigatedToAsync_EmptyFolder_ShowsNoResultsMessage`、英語文言期待のテストが日本語文言を受け取って失敗）はフェーズ20で記録済みの `LocalizationManager` の既知の flaky な事象で、`-parallel none` で実行すると発生しないことを確認済み。`WdV3TimmBuildServiceTests` が実際に `cmd.exe` サブプロセスを起動し `WaitForExitAsync` で待機する分スレッドプールの利用状況が変わり、xUnit v3 の並列実行下でこの既知の flaky 事象が顕在化しやすくなったと考えられるが、本フェーズの変更によるコードの不具合ではない）
  - 本追加修正の完了時点でも `ComfyUILibs`/`ComfyUICaptioningTool` いずれもコミットしていない（ユーザー指示「実装完了後はコミットしないでください」を継続して適用）

### フェーズ33: wdv3-timm 必要最小限ファイルのビルド出力への自動展開（実装完了）

フェーズ32の追加修正で `WdV3TimmPaths.RootDirectory`（`{アプリ実行ファイルと同階層}\wdv3-timm\`）を固定パスとして参照するようにしたが、`wdv3-timm` サブモジュール自体はビルド成果物に含まれておらず、`SettingsPage` の「ビルド」ボタン（`setup.bat`→`build_exe.bat` を実行）を押しても `wdv3-timm` フォルダごと手動でコピーしない限り動作しなかった。`dotnet build`/`dotnet publish` のたびに自動的に展開されるようにした。

- `ComfyUICaptioningTool.csproj` に `Content` の `ItemGroup` を追加し、`..\wdv3-timm\` 配下から以下の5ファイルのみを `Link` メタデータで出力先 `wdv3-timm\` フォルダへ再配置しつつ `CopyToOutputDirectory=PreserveNewest` でビルド出力（`bin\...\win-x64\wdv3-timm\`）・publish 出力（`bin\...\win-x64\publish\wdv3-timm\`）双方へコピーするようにした
  - `setup.bat`（`.venv` 作成・`requirements.txt` インストール）
  - `build_exe.bat`（`.venv` の PyInstaller で `launcher.py` を `wdv3_timm.exe` にビルド）
  - `requirements.txt`（`setup.bat` が参照）
  - `launcher.py`（`build_exe.bat` がビルド対象とするエントリポイント）
  - `wdv3_timm.py`（ビルド後の `wdv3_timm.exe` が実行時にサブプロセスとして呼び出す本体スクリプト）
- 意図的に含めなかったもの: `.venv`（ユーザー環境で `setup.bat` により都度作成）・`README.md`/`CLAUDE.md`/`.claude/`/`.vscode/`/`.gitignore`（ドキュメント・エディタ設定、実行時に不要）・`setup.sh`（Linux 用、Windows 配布物には不要）・`dist`/`build`/`wdv3_timm.spec`（`build_exe.bat` が生成する中間生成物、同バッチが自動削除する）
- 各ファイルの出力先は `<Link>wdv3-timm\%(Filename)%(Extension)</Link>`（複数ファイルを1つの `Content` 要素にまとめてバッチ処理。`%(Filename)`/`%(Extension)` は MSBuild の既知アイテムメタデータのため項目ごとに自動解決される）で明示している。ソースがプロジェクトディレクトリ外（`..\wdv3-timm\`）にあるため `Link` を指定しないと出力先パスが不定になる
- `dotnet build ComfyUICaptioningTool/ComfyUICaptioningTool.csproj` と `dotnet publish ComfyUICaptioningTool/ComfyUICaptioningTool.csproj -c Release` の両方を実行し、それぞれの出力ディレクトリ直下の `wdv3-timm\` に上記5ファイルのみが展開されることを確認済み
- 本フェーズは csproj のみの変更（新規クラスの追加・既存クラスの振る舞い変更を伴わない）のため、ユニットテストの追加は対象外とした。`dotnet build ComfyUICaptioningTool.sln` の成功と、既存テストスイート（337件、既知のクリップボード関連2件を除き回帰なし、`-parallel none` で確認）を確認した
- 本フェーズの変更もユーザー指示「実装完了後はコミットしないでください」を継続して適用し、未コミットのまま作業ツリーに残している

### フェーズ34: SettingsPage「ビルド」実行時のクロススレッド例外の修正（実装完了）

実際に「ビルド」ボタンを押すと `System.NotSupportedException`（「この型の CollectionView は、Dispatcher スレッドとは異なるスレッドからその SourceCollection への変更をサポートしません。」）が `WdV3TimmBuildLogEntries`（UI にバインド済みの `ObservableCollection<string>`）への `Add` 呼び出しで発生する不具合が報告された。

- **原因**: `WdV3TimmBuildService.RunScriptAsync` が `Process.OutputDataReceived`/`ErrorDataReceived` のイベントハンドラーから `outputProgress.Report(e.Data)` を直接呼んでいたが、これらのイベントは `AsyncStreamReader` が管理する専用のスレッドプールのスレッドから発火し、呼び出し元の非同期メソッドの `await` チェーン（`SynchronizationContext` を捕捉して UI スレッドへ戻る仕組み）とは無関係に実行される。`SettingsViewModel.BuildWdV3TimmAsync` はここへ `SynchronousProgress<T>`（`Report` を同期的にそのままコールバックへ委譲する自前実装、`MainPageViewModel` と同じパターン）を渡していたため、UI スレッド以外から `WdV3TimmBuildLogEntries.Add` が呼ばれてしまっていた。`SynchronousProgress<T>` は「`Report` の呼び出しはすべて UI スレッドの `await` 継続から行われる」という前提があるコードで初めて安全になるパターンであり、`MainPageViewModel`（`CaptioningService.ProcessDirectoryAsync` 内で同期的に `progress.Report` を呼ぶ設計）では成立していたが、`WdV3TimmBuildService` のようにプロセスの標準出力イベントから直接呼ぶケースでは成立しない
- **修正**: `WdV3TimmBuildService.RunScriptAsync`（`ComfyUICaptioningTool/Services/WdV3TimmBuildService.cs`）を変更し、`OutputDataReceived`/`ErrorDataReceived` のハンドラーでは `System.Threading.Channels.Channel<string>` へ書き込むだけに留め、`outputProgress.Report` の呼び出しはこのメソッド自身の `await foreach (var line in lines.Reader.ReadAllAsync(cancellationToken))` ループから行うようにした。このループの継続は（他の `await` と同様）呼び出し元が捕捉した `SynchronizationContext` 経由で戻ってくるため、常に呼び出し元スレッド（本番コードでは UI スレッド）で `Report` が呼ばれるようになる。両ストリームの EOF（`e.Data == null`）を `Interlocked.Decrement` でカウントし、両方閉じた時点で `Channel` の Writer を `TryComplete()` する
- **設計判断（`SettingsViewModel.SynchronousProgress<T>` 自体は変更しない）**: 呼び出し元（`WdV3TimmBuildService`）側で「`Report` は必ず呼び出し元スレッドから行われる」という前提を回復させる方が、`ViewModel` 側を `System.Progress<T>`（`SynchronizationContext.Post` 経由の非同期配送）に変更するよりも既存のテスト（`SettingsViewModelTests` の `BuildWdV3TimmCommand_Execute_*` 系、完了直後に同期的に `WdV3TimmBuildLogEntries` の内容を検証する）と整合的であるため、この方針を採った
- **回帰テスト**: `ComfyUICaptioningToolTests/Services/WdV3TimmBuildServiceTests.cs` に `BuildAsync_ProgressReportedOnCallingStaThread_DoesNotThrowCrossThreadCollectionException` を追加。既存の `RecordingProgress<T>`（呼び出し元スレッドを問わず単に記録するだけ）ではこのクロススレッド問題を検出できないため、STA スレッド上で `CollectionViewSource.GetDefaultView` により `ObservableCollection<string>` に対する `CollectionView`（`SettingsPage.xaml` の `ListBox` バインディングが実行時に生成するのと同種のオブジェクト）を生成し、`SynchronousProgress<T>` と同じ同期委譲パターンの `IProgress<T>` 実装からそのコレクションへ直接 `Add` するテストを `TestSupport.StaTestRunner` 経由で実行する形にした。修正前のコードに戻して実行すると、ユーザー報告と同一のスタックトレース（`AsyncStreamReader.FlushMessageQueue` → `ObservableCollection.OnCollectionChanged` → `CollectionView.OnCollectionChanged` → `NotSupportedException`）で失敗することを確認した上で、修正を再適用してテストが通ることを確認した
- 全338件、全件パス確認済み（`ComfyUICaptioningToolTests.exe` 直接実行で確認。今回はクリップボード関連の既知の失敗も含めて回帰なし）
- 本フェーズの変更もユーザー指示「実装完了後はコミットしないでください」を継続して適用し、未コミットのまま作業ツリーに残している

### フェーズ35: SettingsPage のマウスホイールスクロール不具合の修正（実装完了）

「SettingsPage がマウスホイールでスクロールしない」という報告を受けて修正した。フェーズ34の作業中にユーザー（または lint）が「wdv3-timm 実行環境」カードのビルドログ表示を `ListBox` から `ui:ListView`（`ScrollViewer.CanContentScroll`/`ScrollViewer.VerticalScrollBarVisibility` の添付プロパティで内部スクロールを有効化）へ変更していたため、`SettingsPage` 全体を包む外側の `ScrollViewer` の中に、独自にスクロール可能な内側の `ScrollViewer`（`ui:ListView` の内部テンプレートが保持するもの）が入れ子になっていた。WPF の `ScrollViewer` は既定でマウスホイールイベントを（スクロールの必要がなくても）自身で消費してしまうため、ビルドログ領域にマウスカーソルがある状態でホイール操作をすると、外側の `ScrollViewer` までイベントがバブルせずページ全体のスクロールが機能しなくなっていた。`GalleryPage` の右ペインタグ一覧（フェーズ23で導入）で既に同種の問題が発生・修正済みだった（`TagsScrollViewer_PreviewMouseWheel`）ため、同じ対処方針を踏襲した。

- `Views/Pages/SettingsPage.xaml`: ビルドログ表示を `ui:ListView`（添付プロパティによる暗黙のスクロール）から、明示的な `<ScrollViewer x:Name="WdV3TimmLogScrollViewer" PreviewMouseWheel="...">` + `<ItemsControl>` の組み合わせへ変更した（`GalleryPage.xaml` の `TagsScrollViewer` と同じ構成）。表示内容・見た目（`Consolas` フォント・行間マージン等）は変更していない
- `Views/Pages/SettingsPage.xaml.cs`: `WdV3TimmLogScrollViewer_PreviewMouseWheel`（`GalleryPage.xaml.cs` の `TagsScrollViewer_PreviewMouseWheel` と全く同じロジック）を新設。内側の `ScrollViewer` がスクロール端（先頭/末尾）に達している場合のみ、`PreviewMouseWheel` イベントを消費した上で新しい `MouseWheelEventArgs`（`RoutedEvent = UIElement.MouseWheelEvent`）を親要素から再度 `RaiseEvent` し、外側の `ScrollViewer` へ手動でバブルさせる
- 本フェーズは XAML・コードビハインドの表示ロジックのみの変更（新規クラス追加・既存クラスの振る舞い変更を伴わない）のため、ユニットテストの追加は対象外とした（`GalleryPage` の `TagsScrollViewer_PreviewMouseWheel` 追加時と同じ扱い）。`dotnet build ComfyUICaptioningTool.sln` の成功と、既存テストスイート（338件、既知のクリップボード関連2件を除き回帰なし）を確認した
- **既知の関連事項（本フェーズの対応範囲外）**: `DataPage.xaml`（実行結果ログ一覧、各カード内の 1 ファイルごとの処理結果ログ `ListBox`）も同じ「外側 ScrollViewer に入れ子の ListBox」という構成のため、理論上は同じ問題を抱えている可能性がある。今回は SettingsPage のみが報告されたためそちらのみ対応した
- 本フェーズの変更もユーザー指示「実装完了後はコミットしないでください」を継続して適用し、未コミットのまま作業ツリーに残している

### フェーズ36: SettingsPage 全体がスクロールできない不具合の追加修正（実装完了）

フェーズ35の対応後も「SettingsPage 全体がスクロールできない」という報告が続いた。原因はフェーズ35と同時期にページ最上位の `<ScrollViewer>` へ `CanContentScroll="True"` が追加されていたことだった。`CanContentScroll="True"` は `IScrollInfo` を実装したコンテンツ（`VirtualizingStackPanel` 等、`ItemsControl` の仮想化パネルが実装する）に対してのみ意味を持つ設定で、直下の子要素がこれを実装していない場合（本ページの `<StackPanel>` は素の `StackPanel` で `IScrollInfo` 非実装）、`ScrollViewer` はホイール操作・スクロールバー操作いずれによってもコンテンツをスクロールできなくなる（WPF の既知の挙動）。フェーズ35のマウスホイール入れ子問題の修正とは別原因であり、フェーズ35の修正だけでは解消しなかった。

- `Views/Pages/SettingsPage.xaml`: ページ最上位の `<ScrollViewer>` から `CanContentScroll="True"` を削除し、既定値（`False`、ピクセル単位の通常スクロール）に戻した。他の全ページ（`MainPage`/`DataPage`/`GalleryPage`/`ReportPage`/`ConfigPage`）の最上位 `ScrollViewer` もこの属性を指定していない（既定のまま）ことと整合させた
- フェーズ35で追加した `WdV3TimmLogScrollViewer_PreviewMouseWheel`（入れ子スクロール対策）はそのまま維持している
- 本フェーズも XAML のみの変更のためユニットテストの追加は対象外とした。`dotnet build ComfyUICaptioningTool.sln` の成功と、既存テストスイート（338件、既知のクリップボード関連2件を除き回帰なし）を確認した
- 本フェーズの変更もユーザー指示「実装完了後はコミットしないでください」を継続して適用し、未コミットのまま作業ツリーに残している

### フェーズ37: SettingsPage 全体がスクロールできない不具合の真因修正（実装完了）

フェーズ36の対応後も「スクロールバーのドラッグでは動くが、マウスホイールだけ効かない」という具体的な症状が継続していた。この切り分け（スクロールバー操作は機能、ホイールのみ不可）を手掛かりに再調査した結果、フェーズ35・36の対応はいずれも症状の一部を緩和したに過ぎず、真因は `Wpf.Ui.Controls.NavigationView` 自体の既知の未修正の挙動だったことが判明した。

- **真因**: `Wpf.Ui.Controls.NavigationView.NavigationViewContentPresenter`（Wpf.Ui v4.3.0 内部実装）は `ScrollViewer.CanContentScrollProperty.OverrideMetadata(typeof(Page), new FrameworkPropertyMetadata(true))` を実行しており、これによりアプリ内の**全ての `Page`** の `ScrollViewer.CanContentScroll` 添付プロパティの既定値が `true` に上書きされる。さらに `NavigationView` は現在表示中の `Page` のこの値を読み取り、**自身が内部に保持する組み込みの `ScrollViewer`**（`NavigationView` がホストする Page 全体を包むもの）へ反映する（[lepoco/wpfui#1041](https://github.com/lepoco/wpfui/issues/1041)、未解決の既知のライブラリ側の挙動として報告されている）。このため `SettingsPage.xaml` に独自の `<ScrollViewer>` を用意すると、`NavigationView` 組み込みの `ScrollViewer`（外側、実際にスクロールバーが表示される方）の**内側**にもう1つ `ScrollViewer` が入れ子になる二重構造になっていた。WPF の `ScrollViewer` は既定でマウスホイールイベントを（自身がスクロール可能かどうかに関わらず）常に自身で消費してしまうため（[dotnet/wpf#8353](https://github.com/dotnet/wpf/issues/8353) と同種の既知の挙動）、内側（`SettingsPage` 自身の `ScrollViewer`）が常にホイールイベントを吸収し、外側（`NavigationView` 組み込み、実際にスクロールバーが表示される方）まで届かなくなっていた。一方スクロールバーのドラッグは、目に見えるスクロールバー（`NavigationView` 側）を直接マウスでヒットテスト・操作する経路のため、この入れ子構造の影響を受けず正常に機能していた——というのが「スクロールバーは効くがホイールだけ効かない」という報告と正確に一致する
  - フェーズ35（`ui:ListView` → 明示的な `ScrollViewer` + `PreviewMouseWheel` 転送）は、ビルドログ領域という別の入れ子箇所を修正したものであり、この真因とは無関係（今後も有効な修正のため維持）
  - フェーズ36（`CanContentScroll="True"` の削除）は、`SettingsPage` 自身の `<ScrollViewer>` のローカル値を削除しただけで、`CanContentScroll` は継承プロパティであるため、結局 `Page` 自体の（`NavigationView` によって上書きされた）既定値 `true` をそのまま継承してしまっており、実質的に無効化されていなかった
- **修正**: `SettingsPage.xaml` 自身が持っていた独自の `<ScrollViewer>` を完全に削除し（直下の `<StackPanel>` を Page の直接のコンテンツにする）、`NavigationView` 組み込みの `ScrollViewer` にスクロールを一任する構成へ変更した。あわせて `<Page>` ルート要素に `ScrollViewer.CanContentScroll="False"`（添付プロパティ）を明示指定した。これは `NavigationView` が「現在表示中の Page の `CanContentScroll` 値」を読み取って自身の組み込み `ScrollViewer` へ反映する仕組みを逆手に取ったもので、`SettingsPage` の内容（`IScrollInfo` を実装しない素の `StackPanel`）に適した物理（ピクセル単位）スクロールを `NavigationView` 側の `ScrollViewer` に行わせるための指定
- フェーズ35で追加した `WdV3TimmLogScrollViewer_PreviewMouseWheel`（ビルドログ領域の入れ子スクロール対策、`Views/Pages/SettingsPage.xaml.cs`）はそのまま維持している。この入れ子 `ScrollViewer` は今後 `NavigationView` 組み込みの `ScrollViewer` に対して同じ「スクロール端に達した場合のみ親へバブルさせる」役割を果たす
- 本フェーズも XAML のみの変更のためユニットテストの追加は対象外とした。`dotnet build ComfyUICaptioningTool.sln` の成功と、既存テストスイート（338件、既知のクリップボード関連2件を除き回帰なし）を確認した
