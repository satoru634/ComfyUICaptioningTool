...
- `Models/AppConfigTests.cs`・`Services/CaptioningRunResultStoreTests.cs`・`ViewModels/Pages/MainPageViewModelTests.cs`・`ViewModels/Pages/DataViewModelTests.cs`・`ViewModels/Pages/ConfigViewModelTests.cs`・`ViewModels/Pages/SettingsViewModelTests.cs`・`ViewModels/Windows/MainWindowViewModelTests.cs`・`Helpers/LocalizationManagerTests.cs` が存在する（`Fakes/` に `FakeSnackbarService`・`FakeCaptioningService` を用意）。
